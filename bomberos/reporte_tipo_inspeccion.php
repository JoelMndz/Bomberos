...
// Cabecera de página
	function Header() {
		$this->SetTitle(utf8_decode('Listado del Tipo Inspecciones'));
        $this->SetFont('Times', 'B', 25);
		$this->setXY(60, 15);
		$this->Cell(100, 8, 'CUERPO DE BOMBEROS', 0, 1, 'C', 0);
        $this->setXY(60, 25);
		$this->Cell(100, 8, 'DE AZOGUES', 0, 1, 'C', 0);
		$this->SetFont('Times', 'B', 20);
		$this->Image('img/escudo_sin.png', 10, 10, 40); //imagen(archivo, png/jpg || x,y,tamaño)
		$this->setXY(60, 40);
		$this->Cell(100, 8, 'Lista de Tipos de Inspecciones', 0, 1, 'C', 0);
...
// --------------------METODO PARA ADAPTAR LAS CELDAS------------------------------
	...
	function CheckPageBreak($h, $setX) {
		...
		if ($this->GetY() + $h > $this->PageBreakTrigger) {
			...
			$this->Cell(100, 8, utf8_decode('Descripcion'), 1, 0, 'C', 0);
			$this->Cell(80, 8, 'Estado', 1, 0, 'C', 0);
			$this->Cell(40, 8, 'Valor', 1, 1, 'C', 0);
...
$pdf->SetWidths(array(10, 100, 20, 40)); //???
// esto no lo mencione en el video pero también pueden poner la alineación de cada COLUMNA!!!
$pdf->SetAligns(array('C','L','C','L'));

for ($i = 0; $i < count($data); $i++) {
	$pdf->Row(array($i + 1, utf8_decode($data[$i]['descripcion']), 
    ucwords(strtolower(utf8_decode($data[$i]['estado']))),
    $data[$i]['valor']), 15);
}
